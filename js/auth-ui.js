...
async function ensureUserProfile(user) {
  ...
  if (!existing) {
    ...
    await supabase.from('users').insert({
      id: user.id,
      display_name: meta.full_name || meta.name || user.email?.split('@')[0] || 'Anonym',
...
// ── Google OAuth (ab Konfiguration verfügbar) ─────────────
...
export function renderAuthDialog(container) {
  ...
  container.querySelector('#authGoogle').addEventListener('click', async () => {
    ...
    catch (e) { console.error('Google Login:', e.message); }
  });
  ...
  container.querySelector('#authFormRegister').addEventListener('submit', async e => {
    ...
    try {
      ...
      alert('Bitte bestätige deine E-Mail-Adresse. Schau in dein Postfach.');
    } catch (err) {
